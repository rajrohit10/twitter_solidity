...
function getHashedL2ToL1Msg(sender, msg, txNumberInBlock) {
    const encodedMsg = new Uint8Array([
        0,
        1,
        ...ethers_1.ethers.getBytes(ethers_1.ethers.zeroPadBytes(ethers_1.ethers.toBeHex(txNumberInBlock), 2)),
        ...ethers_1.ethers.getBytes(exports.L1_MESSENGER_ADDRESS),
        ...ethers_1.ethers.getBytes(ethers_1.ethers.zeroPadBytes(sender, 32)),
...
function getDeployedContracts(receipt) {
    ...
    return (receipt.logs
        .filter((log) => log.topics[0] == ethers_1.ethers.id("ContractDeployed(address,bytes32,address)") &&
...
function serializeEip712(transaction, signature) {
    ...
    const fields = [
        ...
        transaction.to != null ? ethers_1.ethers.getAddress(transaction.to) : "0x",
    ...
    if (meta.customSignature && ethers_1.ethers.getBytes(meta.customSignature).length == 0) {
    ...
    return ethers_1.ethers.concat([new Uint8Array([exports.EIP712_TX_TYPE]), ethers_1.ethers.encodeRlp(fields)]);
}
...
function parseEip712(payload) {
    ...
    if ((ethers_1.ethers.hexlify(ethSignature.r) == "0x" || ethers_1.ethers.hexlify(ethSignature.s) == "0x") &&
...
function getSignature(transaction, ethSignature) {
    ...
    const r = ethers_1.ethers.getBytes(ethers_1.ethers.zeroPadBytes(ethSignature.r, 32));
    const s = ethers_1.ethers.getBytes(ethers_1.ethers.zeroPadBytes(ethSignature.s, 32));
...
function applyL1ToL2Alias(address) {
    return ethers_1.ethers.hexlify(ethers_1.ethers.toBeHex((BigInt(address) + BigInt(exports.L1_TO_L2_ALIAS_OFFSET)) % ADDRESS_MODULO));
}
...
function undoL1ToL2Alias(address) {
    ...
    return ethers_1.ethers.hexlify(ethers_1.ethers.toBeHex(result));
}
...
function isECDSASignatureCorrect(address, msgHash, signature) {
    try {
        return address == ethers_1.ethers.recoverAddress(msgHash, signature);
    }
    catch {
...
async function isSignatureCorrect(provider, address, msgHash, signature) {
    ...
    isContractAccount = ethers_1.ethers.getBytes(code).length != 0;
...
async function estimateDefaultBridgeDepositL2Gas(providerL1, providerL2, token, amount, to, from, gasPerPubdataByte) {
    ...
    if (token == exports.ETH_ADDRESS) {
        ...
        const l1WethBridge = typechain_1.IL1Bridge__factory.connect(bridgeAddresses.wethL1, providerL1);
        let l2WethToken = ethers_1.ethers.ZeroAddress;
        try {
