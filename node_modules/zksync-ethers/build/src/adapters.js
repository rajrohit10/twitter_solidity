...
function AdapterL1(Base) {
    return class Adapter extends Base {
        ...
        async getMainContract() {
            ...
            return typechain_1.IZkSync__factory.connect(address, this._signerL1());
        }
        async getL1BridgeContracts() {
            const addresses = await this._providerL2().getDefaultBridgeAddresses();
            return {
                erc20: typechain_1.IL1Bridge__factory.connect(addresses.erc20L1, this._signerL1()),
                weth: typechain_1.IL1Bridge__factory.connect(addresses.wethL1, this._signerL1()),
            };
        }
        ...
        async getAllowanceL1(token, bridgeAddress, blockTag) {
            if (!bridgeAddress) {
                ...
                bridgeAddress =
                    l2WethToken != ethers_1.ethers.ZeroAddress
        ...
        async getBaseCost(params) {
            ...
            return await zksyncContract.l2TransactionBaseCost(parameters.gasPrice, parameters.gasLimit, parameters.gasPerPubdataByte);
        }
        async deposit(transaction) {
            ...
            if (transaction.token == utils_1.ETH_ADDRESS) {
                ...
                if (transaction.approveERC20) {
                    ...
                    const proposedBridge = l2WethToken != ethers_1.ethers.ZeroAddress
                        ? await bridgeContracts.weth.getAddress()
                        : await bridgeContracts.erc20.getAddress();
        ...
        async getDepositTx(transaction) {
            ...
            if (tx.bridgeAddress != null) {
                const customBridgeData = tx.customBridgeData ?? (await bridgeContracts.weth.getAddress()) == tx.bridgeAddress
                    ? "0x"
                    : await (0, utils_1.getERC20DefaultBridgeData)(tx.token, this._providerL1());
                let bridge = typechain_1.IL1Bridge__factory.connect(tx.bridgeAddress, this._signerL1());
            ...
            // @ts-ignore
            await gasPriceForEstimation, tx.l2GasLimit, tx.gasPerPubdataByte);
            if (token == utils_1.ETH_ADDRESS) {
                ...
                let refundRecipient = tx.refundRecipient ?? ethers_1.ethers.ZeroAddress;
                const args = [
                    to,
                    token,
                    amount,
                ...
                overrides.value ?? (overrides.value = baseCost + BigInt(operatorTip));
                await (0, utils_1.checkBaseCost)(baseCost, overrides.value);
                overrides.from ?? (overrides.from = await this.getAddress());
                let l2WethToken = ethers_1.ethers.ZeroAddress;
                try {
                    l2WethToken = await bridgeContracts.weth.l2TokenAddress(tx.token);
                }
                catch (e) { }
                const bridge = l2WethToken != ethers_1.ethers.ZeroAddress ? bridgeContracts.weth : bridgeContracts.erc20;
        ...
        async getFullRequiredDepositFee(transaction) {
            ...
            const baseCost = await zksyncContract.l2TransactionBaseCost(
            // @ts-ignore
            gasPriceForMessages, l2GasLimit, tx.gasPerPubdataByte);
            ...
            if (baseCost >= selfBalanceETH + dummyAmount) {
                const recommendedETHBalance = BigInt(tx.token == utils_1.ETH_ADDRESS
            ...
            if (tx.overrides.gasPrice) {
                ...
                fullCost.maxPriorityFeePerGas = BigInt((await tx.overrides.maxPriorityFeePerGas));
            }
            return fullCost;
        }
        ...
        async finalizeWithdrawal(withdrawalHash, index = 0, overrides) {
            ...
            if ((0, utils_1.isETH)(sender)) {
                const withdrawTo = ethers_1.ethers.dataSlice(message, 4, 24);
                ...
                const zksync = typechain_1.IZkSync__factory.connect(contractAddress, this._signerL1());
            ...
            const l1Bridge = typechain_1.IL1Bridge__factory.connect(await l2Bridge.l1Bridge(), this._signerL1());
        ...
        async isWithdrawalFinalized(withdrawalHash, index = 0) {
            ...
            const l2Bridge = typechain_1.IL2Bridge__factory.connect(sender, this._providerL2());
            const l1Bridge = typechain_1.IL1Bridge__factory.connect(await l2Bridge.l1Bridge(), this._providerL1());
        ...
        async claimFailedDeposit(depositHash, overrides) {
            ...
            const l1Bridge = typechain_1.IL1Bridge__factory.connect(l1BridgeAddress, this._signerL1());
        ...
        async getRequestExecuteTx(transaction) {
            ...
            overrides.value ?? (overrides.value = baseCost + BigInt(operatorTip) + BigInt(l2Value));
            await (0, utils_1.checkBaseCost)(baseCost, overrides.value);
            return await zksyncContract.requestL2Transaction.populateTransaction(contractAddress, l2Value, calldata, l2GasLimit, utils_1.REQUIRED_L1_TO_L2_GAS_PER_PUBDATA_LIMIT, factoryDeps, refundRecipient, overrides);
        }
    };
}
